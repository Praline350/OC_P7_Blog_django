...
class LoginForm(forms.Form):
    ...
    login_username = forms.CharField(
        max_length=63, widget=forms.TextInput(attrs={"placeholder": "Nom utilisateur"}), label=False
    )
    password = forms.CharField(
        max_length=63, widget=forms.PasswordInput(attrs={"placeholder": "Mot de passe"}), label=False
    )


class SignUpForm(UserCreationForm):
    signup_form = forms.BooleanField(widget=forms.HiddenInput, initial=True, required=False)
